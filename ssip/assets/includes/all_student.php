...
$result = $query->fetch();
$sc_name = $result['s_name'];
$select = "SELECT * FROM students WHERE sc_name = '$sc_name'";
$query = $connection->query($select);
								...
								<?php foreach ($result as $ans) { ?>
									...
									<tbody>
										<tr>
											...
											<td><?= $ans['adhar'] ?></td>
											<td><?= $ans['class'] ?></td>
											<td><a href="./update_student.php?id=<?=$ans["id"]?>" target="_self"><button id="edit-btn">Edit</button></a></td>
